...
from cblock.mails.services import send_heirs_finished, send_owner_reminder
...
def get_ailve_contracts(contract_type: str, rpc_endpoint: str, test_network: bool) -> Optional[Tuple[List[Any], Web3]]:
    if contract_type == 'probate':
    ...
    elif contract_type == 'wedding':
        alive_contracts = list(WeddingContract.objects.filter(test_node=test_network))
    else:
        logger.error(f'ALIVE CONTRACTS: Contract type {contract_type} is not supported')
        alive_contracts = []
...
def check_dead_wallets(rpc_endpoint: str, test_network: bool) -> None:
    ...
    :return: None
    """
    alive_contracts, w3 = get_ailve_contracts(
        contract_type='probate',
    ...
    for alive_contract in alive_contracts:
        ...
        if contract.functions.isLostKey().call() and not contract.functions.terminated().call():
            ...
            send_heirs_finished(alive_contract)
